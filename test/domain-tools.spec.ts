...
describe("dokploy_domain create", () => {
  it("posts domain.create with host + only defined optional fields", async () => {
    postMock.mockReturnValueOnce(IO.succeed({ domainId: "d1", host: "x.com", https: true }))
    await tool.execute({
      action: "create",
      host: "x.com",
      applicationId: "a1",
  ...
  it("omits undefined optional fields", async () => {
    postMock.mockReturnValueOnce(IO.succeed({ domainId: "d1", host: "x.com", https: false }))
...
describe("dokploy_domain get/update/delete", () => {
  it("get calls domain.one", async () => {
    getMock.mockReturnValueOnce(IO.succeed({ domainId: "d1", host: "x.com", https: true }))
...
describe("dokploy_domain generate / canGenerateTraefikMe / validate", () => {
  it("generate posts domain.generateDomain with appName + optional serverId", async () => {
    postMock.mockReturnValueOnce(IO.succeed({ domain: "app.traefik.me" }))
  ...
  it("generate omits serverId when absent", async () => {
    postMock.mockReturnValueOnce(IO.succeed({}))
  ...
  it("canGenerateTraefikMe GETs and formats result boolean", async () => {
    getMock.mockReturnValueOnce(IO.succeed(true))
  ...
  it("canGenerateTraefikMe formats false result", async () => {
    getMock.mockReturnValueOnce(IO.succeed(false))
